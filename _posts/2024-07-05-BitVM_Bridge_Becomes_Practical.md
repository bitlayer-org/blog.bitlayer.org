...
Operators are a group of Bitcoin users who also holds a significant amount of BTC and wants to profit from the fund. In BitVM bridge, all transactions must be predefined during the setup phase. Since the recipients of future withdrawal transactions are unknown at this stage, BitVM bridge employs an "front-and-reimbursement" model. Liquidity providers fronts Bob's withdrawal request with his own liquidity. Notice that the amount Bob received will be a little less than Bob's request, and the difference is the service fee that goes to the operator. The operator then reclaims his front from the n-of-n multisig address through a reimbursement process.

### Verifier can be Anyone

Because operator may starts invalid reimbursement to steal fund from the n-of-n multisig address, the BitVM Bridge uses fault proof as its core security mechnism. After the operator initiates the reimbursement process, if no fraud proof is submitted within the dispute time window, the operator can retrieve the corresponding BTC from the BitVM Bridge multisig address. Conversely, if the verifiers identifie the reimbursement as illegal within this window, they can challenge the operator. If the operator fails to provide valid proof within a specified timelock, the verifier will confiscate the BTC locked by the operator as punishment.
...
4. **Take1 Transaction**: The operator will wait for the verifier to verify the submitted side system status and reimbursement behavior. This includes checking whether there is a corresponding Bob withdraw transaction in the side system and whether the reimbursement amount and receiving account correspond one-to-one. If there is no verifier challenge within three days after the Kickoff 2 Transaction confirmation, the operator will broadcast the Take 1 Transaction to withdraw 100 BTC (including 99 BTC and 1 BTC handling fee) from the n-of-n multi-signed address and withdraw the 2 BTC previously used for guarantee.
