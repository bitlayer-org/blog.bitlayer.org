...
1. Each operator needs to prepare a 2-BTC UTXO in reserve before the setup phase. Although this UTXO is not locked, if the operator consumes it outside of the protocol, the Kickoff1 transaction will not be valid any more, and the operator can't front any withdrawal request.
2. In each front-and-reimbursement process, the operator needs to front m BTC (99 BTC in this scenario) and unlock the UTXO used for guarantee (2 BTC in this scenario). The operator can only retrieve the funds after the Take1 or Take2 transaction is confirmed. Retrieving the funds through the Take1 transaction path takes at least 2 weeks and 3 days, while the Take2 transaction path takes at least 4 weeks and 3 days.
